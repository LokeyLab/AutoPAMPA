...
import collections, math, itertools, sys, argparse, time, os, matplotlib, openpyxl
...
def parse_args():
    ...
    parser.add_argument ('-o', '--out', dest='outfolder', default='Expt',
            help='Sets the prefix for all output excel files and all directories generated by the --graph argument.')
    parser.add_argument ('-g', '--graph', dest='graphics', action='store_true',
    ...
    parser.add_argument ('-u', '--gauss', dest='usegauss', action='store_true',
            help='Integrate peaks based on a gaussian fit instead of a simple sum.')
    parser.add_argument ('-m', '--massevents', dest='msevents', type=int, default=1,
    ...
    parser.add_argument ('-i', '--ion', dest='iontype', default='H+',
            type=ion_type, help='Sets the ionization type to search. The default is H+, but accepts Na+, NH4+, or any float value.')
...
def configparse (configfile):
    ...
    job_dict = collections.defaultdict(lambda : {'Expt_Type':None,'Ref':[],'Offset':[],'Don':[],'Donor Offset':[],'Acc':[],'Acceptor Offset':[],'Targets':[],'Names':{},'Corrections':{}})
    expt_types_allowed = ['PAMPA','Integrate','Ratio']
    doing_PAMPA = False
    PAMPA_params = ['Volume of Donor Well (ml)','Volume of Acceptor Well (ml)','Active Surface Area of Membrane (cm^2)','Assay Run Time (s)']
    if 'Parameters' in sheet_names:
        ws = jobfile['Parameters']
        params = {'MZ Precision':None,'Minor Peak Detection Threshold':None,'Peak Bound Detection Sensitivity':None,'Begin Bound Detection Below Fractional Height':None,'Maximum Number of Peaks to Report Per Target':None,'Maximum Expected Peak Width (as fraction of run time)':None,'Savitzky-Golay Smoothing Window':None,'Savitzky-Golay Smoothing Order':None,'Volume of Donor Well (ml)':None,'Volume of Acceptor Well (ml)':None,'Active Surface Area of Membrane (cm^2)':None,'Assay Run Time (s)':None,'Retention Time Window':None}
        cell_list = ws['A']
        r=0
        for c in cell_list:
        ...
        for key,value in params.items():
            if key == 'Retention Time Window' and value is not None:
                ...
                if v != '*':
                    try:
                        ...
                        try:
                            ...
                            v1 = float(v1)
                            if v1 < v0:
                                print('Warning: Retention Time Window formatted incorrectly; the start coordinate of the window must be greater than the end coordinate. The program will continue as if * was given.')
                                badv = True
                            else:
                                params[key]=[v0,v1]
                        except ValueError:
                            print('Warning: Retention Time Window formatted incorrectly; should be given as * for no restriction or \'start end\' without the quotes, where start and end are numerical. The program will continue as if * was given.')
                            badv = True
                    except:
                        print('Warning: Retention Time Window formatted incorrectly; should be given as * for no restriction or \'start end\' without the quotes, where start and end are numerical. The program will continue as if * was given.')
                        badv = True
                ...
                try:
                    if key in ['Maximum Number of Peaks to Report Per Target','Savitzky-Golay Smoothing Window','Savitzky-Golay Smoothing Order']:
                    ...
                    if value is None:
                        ...
                        print('Error: Non-numeric value ({}:{}) given for a numeric parameter.'.format(key,value))
                    sys.exit(1)
    else:
    ...
    if 'Experiment Type' in sheet_names:
        ...
        for row in ws.iter_rows(min_row=2):
            ...
            if rowlist[0] in job_dict.keys():
                print('Warning: Experiment type for experiment {} already defined.'.format(rowlist[0]))
            else:
                if rowlist[1] in expt_types_allowed:
                    ...
                    print('Warning: Experiment type {} is invalid. Allowed experiment types are {}.'.format(rowlist[1],','.join(expt_types_allowed)))
    else:
    ...
    if 'Wells' in sheet_names:
        ...
        for row in ws.iter_rows(min_row=2):
            ...
            if rowlist[2] and rowlist[3]:
                ...
                print('Warning: Unpaired Acceptor or Donor well detected in \'Wells\' sheet at row {}. Skipping unpaired wells.'.format(row[0].row))
    else:
        ...
        for row in ws.iter_rows(min_row=2):
            ...
            if rowlist[3]:
                ...
                try:
                    ...
                    print('Error: Integration bound correction column occupied and formatted incorrectly.')
                    sys.exit(1)
    ...
    for expt in job_dict:
        ...
        if job['Expt_Type'] != 'Integrate':
            if len(job['Acc']) == 0:
                ...
                bad_expt = True
            if job['Ref'] == []:
                print('Warning: No Reference well given for Expt {}. Attempting to use a Donor well instead.'.format(expt))
                if len(job['Don']) > 0:
                    job['Ref'] = [job['Don'][0]]
                    print('Donor well {} used as Reference well for Expt {}.'.format(job['Don'][0],expt))
                else:
                    print('Warning: No Donor wells exist to substitute for the Reference well.')
                    bad_expt = True
        elif job['Ref'] == []:
...
class SIC():
    ...
    """
    def __init__ (self,mass,scan_index,timeaxis,intensity,msevents,bound_sensitivity,maxwidthfrac,gauss):
        self.mass = mass
        self.t = scan_index
        self.i = intensity
        self.time = timeaxis
        ...
        if self.gauss:
            ...
            if n > 2:
                ...
                try:
                    ...
                    print("Error: Peak center outside of integration bounds. Center: {} Bounds: {}".format(origin,bounds))
                    sys.exit(1)
                width = xb[1]-xb[0]
                try:
        ...
        while pints:
            ...
            if not 1 < width < self.max_width:
                ...
                while left > -1:
                    if pints[left] != 0:
                        b = bounds[left]
                        if bounds[i][0] <= b[1] <= bounds[i][1]: #right bound encompassed.
                            if b[0] >= bounds[i][0]: #is subset bound
                                ...
                                if overlap > 0.1: #if overlapped significantly, throw out?
                                    bounds[left]=None
                                    pints[left]=0
                        elif b[0] <= bounds[i][0] and b[1] >= bounds[i][1]: #encompassed by both bounds.
                ...
                while right < len(pints):
                    if pints[right] != 0:
                        b = bounds[right]
                        if bounds[i][1] >= b[0] >= bounds[i][0]: #left bound encompassed
...
class Well():
    ...
    """
    def __init__(self, mzml, msevents, Targets, Parameters, Corrections, Gauss, IonMass):
        self.Gauss = Gauss
        self.mzml = mzml
        self.msevents = msevents
        ...
        self.ChromatogramFeatures = collections.defaultdict(lambda : {'Chromatogram':([],[]),'Smoothed':([],[]),'Peaks':([],[]),'Bounds':[],'Traces':[]})
    ...
    def SICbuild (self):
        ...
        reader = pymzml.run.Reader(self.mzml, MSn_Precision=precision)#, obo_version='4.0.1')
        scan_index = 0
        ...
        for spectrum in reader:
            ...
            for t in sicstobuild:
                ...
                while spectrum.mz[p]+precision > t+self.IonMass > spectrum.mz[p]-precision:
                    #add to list of hits
                    ...
                    while tndx+aheadnum < tlen:
                        if spectrum.mz[p]+precision > sicstobuild[tndx+aheadnum]+self.IonMass > spectrum.mz[p]-precision:
        ...
        AllSIC = []
        for t in sicstobuild:
            timeaxis = [indextoretentiontime[scan_index] for scan_index in simdict[t][0]]
            if self.Parameters['Retention Time Window'] != '*':
                ...
                simdict[t] = tuple([simdict[t][0][beginIdx:endIdx],simdict[t][1][beginIdx:endIdx]])
                timeaxis = timeaxis[beginIdx:endIdx]
            AllSIC.append(SIC(t,simdict[t][0],timeaxis,simdict[t][1],self.msevents,self.Parameters['Peak Bound Detection Sensitivity'],self.Parameters['Maximum Expected Peak Width (as fraction of run time)'],self.Gauss))
        return AllSIC
    ...
    def Process (self):
        ...
        for sic in AllSIC:
            ...
            sic.smooth(self.Parameters['Savitzky-Golay Smoothing Window'],self.Parameters['Savitzky-Golay Smoothing Order'])
            #Pick peaks, then choose which ones to integrate.
            peakindices = sic.peakpick(self.Parameters['Minor Peak Detection Threshold'])
            chosenpeaks = [p for p in peakindices if sic.i[p] > 2000] #Throw out the most egregious noise.
            if not len(chosenpeaks) > 0:
                print("Warning: Could not find peaks for mass {} in well {}.".format(sic.mass,self.mzml))
                continue
            #Find Bounds
            bounds = []
            for index,p in enumerate(chosenpeaks):
                bounds.append(sic.definebounds(p,self.Parameters['Begin Bound Detection Below Fractional Height']))
            chosenpeaks,bounds = sic.deduplicatepeaks(chosenpeaks,bounds)
            if len(chosenpeaks) > self.Parameters['Maximum Number of Peaks to Report Per Target']:
                bounds = sorted(bounds, key=lambda x: sic.i[chosenpeaks[bounds.index(x)]], reverse=True)
                bounds = bounds[:self.Parameters['Maximum Number of Peaks to Report Per Target']]
                chosenpeaks.sort(key=lambda x: sic.i[x], reverse=True)
                chosenpeaks = chosenpeaks[:self.Parameters['Maximum Number of Peaks to Report Per Target']]
            ...
            if self.Corrections[sic.mass] != []:
                for corr in self.Corrections[sic.mass]:
                    ...
                    if newleftbound <= chosenpeaks[peak_index] <= newrightbound:
                        ...
                        print("Ignoring correction {} for well {}. Peak is outside of bounds.".format(corr,self.mzml))
            self.ChromatogramFeatures[sic.mass]['Smoothed']=(sic.time,sic.i)
            self.ChromatogramFeatures[sic.mass]['Peaks']=(([sic.time[p] for p in chosenpeaks],[sic.i[p] for p in chosenpeaks])) 
            for b in bounds: 
                self.ChromatogramFeatures[sic.mass]['Bounds'].append(([sic.time[bo] for bo in b],[sic.i[bo] for bo in b]))
...
class Experiment ():
    ...
    """
    def __init__ (self,expt_name,expt_details,parameters,msevents,gauss,verbosity,IonMass):
    ...
    def Process (self):
        ...
        """
        self.RefWell = Well(self.R[0], self.MSevents, self.Targets, self.Parameters, self.Corrections, self.Gauss, self.IonMass)
        ...
        if self.Expt_Type != 'Integrate':
            ...
            for d,a in zip(self.D,self.A):
                if first_step and self.R[0] == d:
                    ...
                    dwell = Well(d, self.MSevents, self.Targets, self.Parameters, self.Corrections, self.Gauss, self.IonMass)
                    dwell.Process()
                    self.DonWells.append(dwell)
                awell = Well(a, self.MSevents, self.Targets, self.Parameters, self.Corrections, self.Gauss, self.IonMass)
                awell.Process()
                self.AccWells.append(awell)
        else: #Integration only
            for w in self.R[1:]:
                otherwell = Well(w, self.MSevents, self.Targets, self.Parameters, self.Corrections, self.Gauss, self.IonMass)
                otherwell.Process()
    ...
    def AlignPeaks (self):
        ...
        for mass in itertools.chain(self.Targets):
            ...
            master = [self.RefWell.ChromatogramFeatures[mass]['Chromatogram'][0][x] for x in self.RefWell.ChosenPeaks[mass]] 
            #Peak widths in ref well
            mgw = self.RefWell.gaussWidth[mass]
            mbds = self.RefWell.Bounds[mass]
            if self.Gauss:
                mwidths = [mgw[i] if mgw[i] is not None else mbds[i][1]-mbds[i][0] for i in range(len(mgw))]
            else:
                mwidths = [x[1]-x[0] for x in mbds]
            maxt = self.RefWell.ChromatogramFeatures[mass]['Chromatogram'][0][-1]
            maxdist = max(3,math.ceil(maxt/360)) #3 seconds minimum, longer if above 15 minute LC method.
            self.RefWell.Alignment[mass] = list(range(len(self.RefWell.ChosenPeaks[mass])))
            if self.Expt_Type != 'Integrate':
                for d,doff,a,aoff in zip(self.DonWells,self.Doff,self.AccWells,self.Aoff):
                    dtimes = [d.ChromatogramFeatures[mass]['Chromatogram'][0][x]+doff for x in d.ChosenPeaks[mass]]
                    atimes = [a.ChromatogramFeatures[mass]['Chromatogram'][0][x]+aoff for x in a.ChosenPeaks[mass]]
                    dgw = d.gaussWidth[mass]
                    agw = a.gaussWidth[mass]
                    dbds = d.Bounds[mass]
                    abds = a.Bounds[mass]
                    if self.Gauss:
                        dwidths = [dgw[i] if dgw[i] is not None else dbds[i][1]-dbds[i][0] for i in range(len(dgw))]
                        awidths = [agw[i] if agw[i] is not None else abds[i][1]-abds[i][0] for i in range(len(agw))]
                    else:
                        dwidths = [x[1]-x[0] for x in dbds]
                        awidths = [x[1]-x[0] for x in abds]
                    d.Alignment[mass] = self.Pair(maxdist,master,mwidths,dtimes,dwidths)
                    a.Alignment[mass] = self.Pair(maxdist,master,mwidths,atimes,awidths)
            else:
                for e,eoff in zip(self.ExtraWells,self.Eoff):
                    etimes = [e.ChromatogramFeatures[mass]['Chromatogram'][0][x]+eoff for x in e.ChosenPeaks[mass]]
                    egw = e.gaussWidth[mass]
                    ebds = e.Bounds[mass]
                    if self.Gauss:
                        ewidths = [egw[i] if egw[i] is not None else ebds[i][1]-ebds[i][0] for i in range(len(egw))]
                    else:
                        ewidths = [x[1]-x[0] for x in ebds]
                    e.Alignment[mass] = self.Pair(maxdist,master,mwidths,etimes,ewidths)
        ...
        for i,to in enumerate(master):
            ...
            for j,tm in enumerate(other):
                ...
                if dist > maxdist:
                    ...
                    if 0.2 < owidths[j]/mwidths[i] < 2: #If the peak width has not changed by 5-fold or more.
                        mindist = dist
                        minind = j
            alignment[i]=minind
        return alignment
...
def singlewellplot (uberplot,xlims,mass,well,numplots,plotnum,usegauss,verbose):
    ...
    if verbose >= 2:
        ...
        ax.plot(*well.ChromatogramFeatures[mass]['Chromatogram'],color='b',linestyle='-')
    if usegauss:
        if well.gaussI[mass] != None:
            for trace in well.ChromatogramFeatures[mass]['Traces']:
                if trace:
                    ax.plot(*trace,color='r',linestyle='--', linewidth=1.3)
    ax.plot(*well.ChromatogramFeatures[mass]['Peaks'],color='k',marker='D',linestyle=' ',markersize=4)
        ...
        for x,y,a in zip(*well.ChromatogramFeatures[mass]['Peaks'],ann):
            if a != None: 
                if y > annheightthresh:
                    ax.annotate('{},({:.0f},{:.2E})'.format(a+1,x,y), xy=(x,y), xytext=(x,1.05*y), textcoords='data', fontsize=6)
    for b in well.ChromatogramFeatures[mass]['Bounds']:
        ax.plot(*b,color='r',marker='D',linestyle=' ',markersize=4)
    if plotnum == 1:
        peakproxy = matplotlib.lines.Line2D([],[],color='k',marker='D',linestyle=' ',label='Peaks Picked')
        boundproxy = matplotlib.lines.Line2D([],[],color='r',marker='D',linestyle=' ',label='Bounds of Integration')
        if usegauss:
            traceproxy = matplotlib.lines.Line2D([],[],color='r',linestyle='--', linewidth=2.0, label='Fit')
...
def scoreprint (outfolder,Parameters,Expt_list,usegauss,graphics):
    ...
    compIntList=['Experiment','Name','Mass','Peak Number','Rt (s)','Ref: filename', 'Ref: intensity','Bounds of Integration']
    compPAMPAList=['Experiment','Name','Mass','Peak Number','Rt (s)','Ref: filename', 'Ref: intensity',  'Bounds of Integration']
    #'Donor: filename', 'Donor: intensity', 'Acceptor: filename', 'Acceptor: intensity', '%T', '%R', 'PeE-6', 'Bounds of Integration'])
    compRatioList=['Experiment','Name','Mass','Peak Number','Rt (s)','Ref: filename', 'Ref: intensity', 'Bounds of Integration']
    
    #Set up column names for humanout sheets
    humIntList =['Experiment','Name','Mass','Peak Number','Rt (s)','Average','Standard Deviation','Bounds of Integration']
    humPAMPAList =['Experiment','Name','Mass','Peak Number','Rt (s)','Avg%T','Avg%R','AvgPeE-6','StdDevPeE-6','Recovery Well Intensity','Donor Well Intensity', 'Acceptor Well Intensity','Bounds of Integration']
    humRatioList = ['Experiment','Name','Mass','Peak Number','Rt (s)','Well A Average','Well A Standard Deviation','Well B Average','Well B Standard Deviation','Ratio(A/B)','LogRatio(A/B)','Bounds of Integration']
    ...
    for i in numwells[0]:
        compPAMPAList.extend(['Donor: filename', 'Donor: intensity', 'Acceptor: filename', 'Acceptor: intensity', '%T', '%R', 'PeE-6'])
        compRatioList.extend([ 'Well A: filename', 'Well A: intensity', 'Well B: filename', 'Well B: intensity', 'Ratio'])
    ...
    for exp in Expt_list:
        ...
        AccWells = exp.AccWells
        ExtraWells = exp.ExtraWells
        integrationdict = collections.defaultdict(lambda: {'refint':[],'dint':[],'aint':[],'eint':[]})
        ...
        if exp.Expt_Type == 'Integrate':
            ...
            for mass in exp.Targets:
                for p in range(len(RefWell.TotalI[mass])):
                    ...
                    complist.extend([mass,p+1,RefWell.ChromatogramFeatures[mass]['Peaks'][0][p],RefWell.mzml,refint])
                    complist.extend(['{:.2f}, {:.2f}'.format(*boundprint)])
                    humlist.extend([mass, p+1,RefWell.ChromatogramFeatures[mass]['Peaks'][0][p]])
                    for i,e in enumerate(ExtraWells):
                        eint = integrationdict[mass]['eint'][p][i]
                        complist.append(e.mzml)
                        if eint: #Undetected peaks not reported as zero, as it's assumed those runs are bad and should be left out.
                    ...
                    if len(elist) > 0:
                        ...
                        humlist.append(None)
                    if len(elist) > 1:
                        stdevint = math.sqrt(sum([(x-averageint)**2 for x in elist])/(len(elist)-1)) #corrected sample standard deviation
                    ...
                    if graphics:
                        humlist.append(make_link_cell(humsheet,'=HYPERLINK(CONCATENATE(LEFT(CELL(\"filename\"),FIND(\"[\",CELL(\"filename\"),1)-1),\"{}\\Chromatograms for mass {}.svg\"),\"link\")'.format(outfolder+'_'+exp.Expt,mass)))
            ...
            for mass in exp.Targets:
                for p in range(len(RefWell.TotalI[mass])):
                    ...
                    if exp.Names[mass]:
                        ...
                        humlist.append(None)
                    boundprint = RefWell.ChromatogramFeatures[mass]['Bounds'][p][0]
                    complist.extend([mass, p+1, RefWell.ChromatogramFeatures[mass]['Peaks'][0][p], RefWell.mzml, refint,'{:.2f}, {:.2f}'.format(*boundprint)])
                    humlist.extend([mass, p+1, RefWell.ChromatogramFeatures[mass]['Peaks'][0][p]])
                    percentTs = []
                    percentRs = []
                    Pes = []
                    for j,both in enumerate(zip(AccWells, DonWells)):
                        ...
                        if dint and dint!=0: 
                            ...
                            #Percent T
                            percentT = 100*(aint / ((dint*Vd+aint*Va) / (Vd+Va)))
                            complist.append(percentT)
                            percentTs.append(percentT)
                            #Percent R
                            if refint and refint != 0:
                                percentR = 100*((dint*Vd+aint*Va)/(refint*Vd))
                                complist.append(percentR)
                                percentRs.append(percentR)
                            else:
                                complist.append(None)
                            #Permeability (cm/s)
                            if percentT >= 100: #Still output this so that the hyperlink to graphics are in the sheet.
                                ...
                                Pe = Vd*Va/((Vd+Va)*Area*Ptime)*-1*math.log(1-percentT/100)
                    ...
                    if len(Pes) > 0:
                        ...
                        humlist.append(None)
                    if len(Pes) > 1:
                        stdevPe = math.sqrt(sum([(x-avgPe)**2 for x in Pes])/(len(Pes)-1))
                    ...
                    if numd == 1 and numa == 1:
                        humlist.extend([refint,integrationdict[mass]['dint'][p][0],integrationdict[mass]['aint'][p][0],'{:.2f}, {:.2f}'.format(*boundprint)])
                    else:
                        humlist.extend([refint,sum(integrationdict[mass]['dint'][p])/numd,sum(integrationdict[mass]['aint'][p])/numa,'{:.2f}, {:.2f}'.format(*boundprint)])
                    #humsheet.append(humlist)
                    if graphics:
                        humlist.append(make_link_cell(humsheet,'=HYPERLINK(CONCATENATE(LEFT(CELL(\"filename\"),FIND(\"[\",CELL(\"filename\"),1)-1),\"{}\\Chromatograms for mass {}.svg\"),\"link\")'.format(outfolder+'_'+exp.Expt,mass)))
                    compsheet.append(complist)
                ...
                for p in range(len(RefWell.TotalI[mass])):
                    ...
                    for j,both in enumerate(zip(AccWells,DonWells)):
                        ...
                        if aint and aint != 0:
                            ...
                            complist.append('NaN') #Won't show up in the summary file, but useful to know.
                            dlist.append(dint)
                            alist.append(0)
                        else:
                            complist.append(None)
                    ...
                    if len(dlist) > 0:
                        ...
                        humlist.append(None)
                    if len(dlist) > 1:
                        dstdev = math.sqrt(sum([(x-davg)**2 for x in dlist])/(len(dlist)-1))
                    ...
                    if len(alist) > 1:
                        astdev = math.sqrt(sum([(x-aavg)**2 for x in alist])/(len(alist)-1))
...
def makegraphs (outfolder,Expt_list,usegauss,verbose):
    ...
    for expt in Expt_list:
        ...
        for mass in expt.Targets:
            ...
            pyplot.axis('off')
            xlims = (RefWell.ChromatogramFeatures[mass]['Chromatogram'][0][0],RefWell.ChromatogramFeatures[mass]['Chromatogram'][0][-1])
            #Reference well first
            if expt.Expt_Type != 'Integrate':
                if RefWell != DonWells[0]:
                    singlewellplot(uberplot, xlims, mass, RefWell, numplots, plotnum, usegauss, verbose)
                    plotnum+=1
                for d,a in zip(DonWells,AccWells):
                    singlewellplot(uberplot, xlims, mass, d, numplots, plotnum, usegauss, verbose)
                    plotnum +=1
                    singlewellplot(uberplot, xlims, mass, a, numplots, plotnum, usegauss, verbose)
                    plotnum +=1
            else:
                singlewellplot(uberplot, xlims, mass, RefWell, numplots, plotnum, usegauss, verbose)
                plotnum+=1
                for well in ExtraWells:
                    singlewellplot(uberplot, xlims, mass, well, numplots, plotnum, usegauss, verbose)
                    plotnum +=1
            uberplot.subplots_adjust(left=0.15, bottom=0.05, right=0.97, top=0.92, wspace=0.0, hspace=0.25)
            uberplot.text(0.02, 0.5, 'Intensity', va='center', rotation='vertical', size='large')
            try:
                os.mkdir('{}\\{}_{}\\'.format(cwd,outfolder,expt.Expt))
            except OSError:
                pass
            uberplot.savefig('{}\\{}_{}\\Chromatograms for mass {}.svg'.format(cwd,outfolder,expt.Expt,mass))
...
def main():
    ...
    for key,value in job_dict.items():
        Expt_list.append(Experiment(key,value,params,options.msevents,options.usegauss,options.verbose,options.iontype))
    for ex in Expt_list:
        ex.Process()
    if verbose:
